<template>
  ...
  <div >
    <h1>The Book You May be Interested ...</h1>
...
export default {
  ...
  methods: {
    ...
    getFav() {
      request.get('/book/getFavList').then(res => {
        if (res.code === '200') {
          // this.$notify.success('Add to favourite Book List')
          // console.log(res)
          // console.log(this.tableData)
          this.FavList = []
          this.FavIds = res.data
          let cateSet = new Set()
          for (let i = 0; i < this.FavIds.length ; i++) {
            for(let j= 0; j<this.tableData.length;j++){
              if (this.FavIds[i].bookid === this.tableData[j].id){
                this.tableData[j].rankscore = this.tableData[j].rankscore + 100
                console.log(this.tableData[j].rankscore)
              }
            }
          }
          for (let i = 0; i <this.tableData.length ; i++) {
            if (this.tableData[i].reading || this.tableData[i].finishread){
                cateSet.add(this.tableData[i].category)
            }
          }
          console.log("cateSet",cateSet)
          for (let i = 0; i <this.tableData.length ; i++) {
            if (cateSet.has(this.tableData[i].category) ){
              this.tableData[i].rankscore += 10
            }
          }
          ...
          for (let i = 0; i <this.tableData.length ; i++) {
            if (this.tableData[i].toread){
              this.FavList.push(this.tableData[i])
            }
          }
          // console.log(this.tableData)
        } else {
